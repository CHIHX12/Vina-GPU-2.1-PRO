...
void read_file(char** program_file, size_t* program_size, std::string file_path) {
   ...
   fprintf(stderr, "\nError: cannot open OpenCL source file:\n  %s\n"
           "Hint: set VINA_GPU_HOME to the directory containing OpenCL/\n",
           file_path.data());
   exit(-1);
}
...
void SetupPlatform(cl_platform_id** platforms, cl_int* gpu_platform) {
    ...
    *platforms = (cl_platform_id*)malloc(sizeof(cl_platform_id) * (num_platform));
    err = clGetPlatformIDs(num_platform, *platforms, NULL); checkErr(err);
        ...
        for (int i = 0; i < num_platform; i++) {
            err = clGetPlatformInfo((*platforms)[i], CL_PLATFORM_NAME, 0, NULL, &size); checkErr(err);
            // Fix: allocate size+1 and null-terminate
            char* platform_name = (char*)malloc(sizeof(char) * (size + 1));
            err = clGetPlatformInfo((*platforms)[i], CL_PLATFORM_NAME, size, platform_name, NULL); checkErr(err);
            platform_name[size] = '\0';
            std::string tmp = platform_name;
...
int QueryDeviceCount() {
    ...
    cl_int gpu_platform_id = -1;
    SetupPlatform(&platforms, &gpu_platform_id);
    if (gpu_platform_id < 0 || !platforms) return 0;
    cl_uint num_device = 0;
#ifdef INTEL_CPU_PLATFORM
    clGetDeviceIDs(platforms[gpu_platform_id], CL_DEVICE_TYPE_CPU, 0, NULL, &num_device);
#else
    clGetDeviceIDs(platforms[gpu_platform_id], CL_DEVICE_TYPE_GPU, 0, NULL, &num_device);
#endif
    free(platforms);
    return static_cast<int>(num_device);
}
...
void SetupQueue(cl_command_queue* queue, cl_context context, cl_device_id* devices, int device_index) {
    cl_int err = 0;
    cl_command_queue_properties props[] = { CL_QUEUE_PROFILING_ENABLE};
    *queue = clCreateCommandQueue(context, devices[device_index], *props, &err); checkErr(err);
...
void SaveProgramToBinary(cl_program program_cl,const char* file_name) {
    ...
    if (!fp) {
        fprintf(stderr, "Warning: cannot write kernel binary '%s' — skipping cache\n", file_name);
    } else {
        for (int i = 0; i < numDevices; i++) {
...
cl_program SetupBuildProgramWithBinary(cl_context context, cl_device_id* devices, const char* binary_file_name) {
    ...
    // Fix: check fopen result and use RAII-style cleanup on failure
    ...
    // Fix: if malloc fails, close file before exiting
    char* binary_buffer = (char*)malloc(program_size + 1);
    if (!binary_buffer) {
        fclose(program_handle);
        fprintf(stderr, "Error: out of memory reading kernel binary '%s'\n", binary_file_name);
        exit(-1);
    }
    ...
    free(binary_buffer); // Fix: free buffer after program is created
    checkErr(err);
