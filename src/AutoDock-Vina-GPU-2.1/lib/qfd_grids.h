...
// Load a precomputed QFD grid binary into `slot`. Returns false (slot untouched) if the file is
// absent OR if its spatial frame does not cover the current docking box (`ref_grid`) — the latter
// guards against silently applying a stale qfd_*.bin left in the CWD from a different receptor/box.
